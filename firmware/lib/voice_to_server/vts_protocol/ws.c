...
{
    esp_websocket_client_init(&voice_to_server_client_cfg);
...
void voice_to_server_ws_callback(void *data, size_t size)
{
    ...
    {
        esp_websocket_client_send_bin(client, (const char *)data, sizeof(data), portMAX_DELAY);
        ESP_LOGI(TAG, "Sending data : %d bytes", sizeof(data));
    }
}
